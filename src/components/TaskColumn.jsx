...
function TaskColumn({ column }) {
    ...
    return (
        ...
        <section className={`task-column flex-vertical flex-1  max-w-sm rounded p-8 text-center ${column} ${column === 'doing' ? ' bg-blue-400 ' : 'bg-blue-200'}`}>
            <h2 className='column-title font-bold rounded p-2 m-2'>{column.toUpperCase()}</h2>
            <section className='task-list'>
            {
                filteredTasks.map(task => (
                    ...
                    <div key={task.id} className={`task-card w-full bg-blue-100 rounded flex m-2 p-2 justify-between items-center hover:shadow-xl hover:scale-105 transition-all duration-300`}>
                        <section className={`font-bold p-2 ${task.priority === 'high'
                            ? 'text-red-800'
                            : 'text-green-800'}`}>
            ...
            <form onSubmit={e => {
                ...
                <input value={inputText} onChange={e => setInputText(e.target.value)} placeholder="Note's text..." className="p-2 focus:outline-none active:outline-none rounded flex-1 bg-gray-200" />
